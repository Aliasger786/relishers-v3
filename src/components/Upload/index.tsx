...
const Upload = ({ action, promise, progressHandler, to = "Products" }: { action: (img: any) => void; promise: any; progressHandler: any; to?: string }) => {
  ...
  return (
    <div className="flex justify-center items-center w-full h-full">
      <label htmlFor="file-upload" className="flex flex-col justify-center items-center w-full h-full rounded-lg cursor-pointer">
        ...
        <input id="file-upload" name="uploadimage" type="file" accept="image/*" className="hidden" onChange={uploadImage} />
      </label>
    </div>
  );
};
